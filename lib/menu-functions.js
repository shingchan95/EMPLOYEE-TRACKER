...
const viewAllEmployees =() =>{
    db.query('SELECT employee.id, employee.first_name, employee.last_name, employee_role.job_title, department.department_name, employee_role.salary, CONCAT(managers.first_name," ", managers.last_name) AS manager FROM department INNER JOIN employee_role ON department.id = employee_role.department_id INNER JOIN employee ON employee_role.id = employee.employee_role_id LEFT JOIN employee AS managers on employee.manager_id = managers.id', function (err, results) {
        console.log("\n")
        console.table(results);
    });
}
...
const updateEmployeeRole =(titlenameID,employeeID) =>{
    db.query('UPDATE employee INNER JOIN employee_role ON employee.employee_role_id= employee_role.id SET employee.employee_role_id = ? WHERE employee.id= ?',[titlenameID,employeeID], function (err, results) {
        console.log(results);
    });
}
